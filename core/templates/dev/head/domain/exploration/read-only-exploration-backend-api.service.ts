...
export class ReadOnlyExplorationBackendApiService {
  ...
  _fetchExploration(
    ...
    this.httpClient.get(explorationDataUrl).toPromise().then(
      (response: any) => {
        let exploration = cloneDeep(response.data);
        if (successCallback) {
  ...
  _isCached(explorationId) {
    return this._explorationCache.hasOwnProperty(explorationId);
  }

  _getExplorationUrl(explorationId, version) {
    if (version) {
   ...
   */
  fetchExploration(explorationId, version):Promise<any> {
   ...
   */
  loadLatestExploration(explorationId) {
    return new Promise((resolve, reject) => {
   ...
   */
  cacheExploration(explorationId, exploration) {
