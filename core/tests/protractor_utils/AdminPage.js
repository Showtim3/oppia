...
var AdminPage = function() {
  ...
  var unfinishedOffJobRows = element.all(by.css(
  ...
  this.startOneOffJob = function(jobName) {
    ...
    oneOffJobRows.count().then(function(length) {
      let found = false;
  ...
  this.expectOneOffJobTobeRunning = function(jobName) {
    browser.sleep(5000);
  ...
  this.stopOneOffJob = function(jobName) {
    ...
    var index = -1;
    var selectedIndex = 0;
    oneOffJobRows.map(function(row) {
      ...
      oneOffJobRows.get(selectedIndex).element(
  ...
  this.expectNumberOfRunningOneOffJobs = function(count) {
    element.all(by.css(
      '.protractor-test-unfinished-one-off-jobs-id')).count().then(
      function(length) {
  ...
  this._expectJobTobeStarted = function(jobName) {
    return unfinishedOffJobRows.filter(function(row) {
      return row.getText().then(function(text) {
